...
router.post("/categories", requireAdmin, async (req, res) => {
  try {
    ...
    if (!categoryData.image) {
      categoryData.image = "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg";
    }
...
router.get("/restaurant-sections", requireAdmin, async (req, res) => {
  try {
    ...
    console.error("خطأ في جلب أقسام المطاعم:", error);
    res.status(500).json({ error: "خطأ في الخادم" });
  }
});
...
router.post("/restaurants", requireAdmin, async (req, res) => {
  try {
    ...
    if (!restaurantData.logo) {
      restaurantData.logo = restaurantData.image;
    }
...
router.put("/orders/:id/status", requireAdmin, async (req: any, res) => {
  try {
    ...
    await db.insert(schema.orderTracking).values({
      orderId: id,
      status,
      message: `تم تحديث حالة الطلب إلى: ${getStatusMessage(status)}`,
    ...
    // إشعار جميع السائقين بالطلبات الجديدة
    if (status === 'confirmed') {
      broadcastToDrivers({
        type: 'new_order',
        title: 'طلب جديد متاح',
        message: `طلب جديد متاح للتوصيل #${updatedOrder.orderNumber}`,
        orderId: id,
        order: updatedOrder
      });
    }
...
router.post("/special-offers", requireAdmin, async (req, res) => {
  try {
    ...
    // إضافة صورة افتراضية إذا لم تكن موجودة
    if (!offerData.image) {
      offerData.image = "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg";
    }
